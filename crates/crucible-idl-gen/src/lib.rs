...
mod tests {
    ...
    fn parse_test_idl(filename: &str) -> Idl {
        let json = std::fs::read_to_string(
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("test-data").join(filename),
        )
    ...
    fn single_accounts(ix: &anchor_lang_idl::types::IdlInstruction) -> Vec<&anchor_lang_idl::types::IdlInstructionAccount> {
        ix.accounts.iter().filter_map(|a| match a {
    ...
    fn test_parse_codama_system() {
        ...
        verify_idl_basics(&idl);
        assert!(idl.metadata.name.contains("system") || idl.metadata.name.contains("System"),
            "system IDL name: {}", idl.metadata.name);
        // System program has multiple instructions
        assert!(idl.instructions.len() >= 10,
            "system should have >= 10 instructions, got {}", idl.instructions.len());
    }
    ...
    fn test_parse_codama_token() {
        ...
        assert!(idl.instructions.len() >= 20,
            "token should have >= 20 instructions, got {}", idl.instructions.len());
    }
    ...
    fn test_parse_anchor_stake() {
        ...
        let sa = idl.types.iter().find(|t| t.name == "StakeAuthorize")
        ...
        let lockup = idl.types.iter().find(|t| t.name == "Lockup")
            .expect("should have Lockup type");
        match &lockup.ty {
            IdlTypeDefTy::Struct { fields: Some(IdlDefinedFields::Named(f)) } => {
                assert_eq!(f.len(), 3);
        ...
        assert_eq!(accs[1].address.as_deref(), Some("SysvarRent111111111111111111111111111111111"));
    }
    ...
    fn test_parse_anchor_marginfi() {
        ...
        assert!(dep_accs.iter().any(|a| a.name == "marginfi_account" && a.writable));
        assert!(dep_accs.iter().any(|a| a.name == "bank" && a.writable));
        assert!(dep_accs.iter().any(|a| a.name == "liquidity_vault" && a.writable));

        // -- Deep: WrappedI80F48 type structure --
        let wi = idl.types.iter().find(|t| t.name == "WrappedI80F48")
        ...
        match &wi.ty {
            ...
            other => panic!("WrappedI80F48 should be struct with named fields, got {:?}", other),
        }

        // -- Deep: Bank type first 3 fields --
        let bank = idl.types.iter().find(|t| t.name == "Bank")
        ...
        let bank_acc = idl.accounts.iter().find(|a| a.name == "Bank")
    ...
    fn test_parse_anchor_whirlpool() {
        ...
        let has_repr_c = idl.types.iter().any(|t| {
            matches!(&t.repr, Some(anchor_lang_idl::types::IdlRepr::C(_)))
        });
        assert!(has_repr_c, "whirlpool should have repr(C) types");

        // -- Deep: exactly 5 repr(C) types --
        let repr_c_types: Vec<&str> = idl.types.iter()
            .filter(|t| matches!(&t.repr, Some(anchor_lang_idl::types::IdlRepr::C(_))))
            .map(|t| t.name.as_str())
            .collect();
        assert_eq!(repr_c_types.len(), 5,
            "should have 5 repr(C) types, got {:?}", repr_c_types);
        ...
        let wp = idl.types.iter().find(|t| t.name == "Whirlpool")
        ...
        let ta = idl.types.iter().find(|t| t.name == "TickArray")
            ...
            IdlTypeDefTy::Struct { fields: Some(IdlDefinedFields::Named(f)) } => {
                ...
                match &f[1].ty {
                    IdlType::Array(inner, IdlArrayLen::Value(88)) => {
                        assert!(matches!(&**inner, IdlType::Defined { name, .. } if name == "Tick"),
                            "ticks inner type should be Defined(Tick), got {:?}", inner);
                    }
        ...
        let wp_acc = idl.accounts.iter().find(|a| a.name == "Whirlpool")
            .expect("should have Whirlpool account");
        assert_eq!(wp_acc.discriminator, vec![63, 149, 209, 12, 225, 128, 99, 9]);
    }
    ...
    fn test_parse_shank_bubblegum() {
        ...
        assert!(idl.instructions.len() >= 30,
            "bubblegum should have >= 30 instructions, got {}", idl.instructions.len());
        ...
        assert!(ix_names.contains(&"transfer"), "should have transfer instruction");
        // mintV1 → mint_v1 after legacy conversion
        assert!(ix_names.contains(&"mint_v1"), "should have mint_v1 instruction, got {:?}", ix_names);

        // -- Deep: burn instruction args --
        let burn = idl.instructions.iter().find(|ix| ix.name == "burn")
        ...
        let ma = idl.types.iter().find(|t| t.name == "MetadataArgs")
        ...
        match &ma.ty {
            ...
            other => panic!("MetadataArgs should be struct with named fields, got {:?}", other),
        }
    }
    ...
    fn test_parse_shank_candy_guard() {
        ...
        assert!(idl.types.len() >= 20,
            "candy_guard should have >= 20 types, got {}", idl.types.len());

        // -- Deep: Allocation struct --
        let alloc = idl.types.iter().find(|t| t.name == "Allocation")
    ...
    fn test_bincode_discriminators_truncated_codama_stake() {
        ...
        for ix in &idl.instructions {
            assert_eq!(ix.discriminator.len(), 4,
                "instruction {} should have 4-byte discriminator after truncation", ix.name);
        }
    ...
    fn test_anchor_discriminators_not_truncated() {
        ...
        for ix in &idl.instructions {
            assert_eq!(ix.discriminator.len(), 8,
                "instruction {} should keep 8-byte discriminator", ix.name);
        }
        // Verify that last 4 bytes are not all zero (statistically certain for SHA256)
        let has_nonzero_tail = idl.instructions.iter().any(|ix| {
            ix.discriminator[4..] != [0, 0, 0, 0]
        });
        assert!(has_nonzero_tail, "SHA256 discriminators should have non-zero last 4 bytes");
    }
    ...
    fn test_codama_anchor_stake_equivalence() {
        ...
        assert_eq!(codama.instructions.len(), anchor.instructions.len(),
            "instruction count mismatch");

        // Same discriminator values after truncation
        for (c, a) in codama.instructions.iter().zip(&anchor.instructions) {
            assert_eq!(c.discriminator, a.discriminator,
                "discriminator mismatch for codama:{} vs anchor:{}", c.name, a.name);
        }

        // Both have StakeAuthorize with same variants
        let c_sa = codama.types.iter().find(|t| t.name == "StakeAuthorize")
            .expect("codama should have StakeAuthorize");
        let a_sa = anchor.types.iter().find(|t| t.name == "StakeAuthorize")
        ...
        let c_lockup = codama.types.iter().find(|t| t.name == "Lockup")
            .expect("codama should have Lockup");
        let a_lockup = anchor.types.iter().find(|t| t.name == "Lockup")
            .expect("anchor should have Lockup");
        match (&c_lockup.ty, &a_lockup.ty) {
            (
                IdlTypeDefTy::Struct { fields: Some(IdlDefinedFields::Named(cf)) },
                IdlTypeDefTy::Struct { fields: Some(IdlDefinedFields::Named(af)) },
            ) => {
    ...
    fn test_pipeline_codama_stake() {
        ...
        assert!(output.contains("mod instruction"), "should have instruction module");
        assert!(output.contains("mod accounts"), "should have accounts module");
        ...
        assert!(output.contains("StakeAuthorize"), "should have StakeAuthorize type");
        // Should have specific instructions
        assert!(output.contains("Initialize"), "should have Initialize instruction");
        assert!(output.contains("DelegateStake"), "should have DelegateStake instruction");
    }
    ...
    fn test_pipeline_codama_system() {
        ...
        assert!(output.contains("CreateAccount"), "should have CreateAccount instruction");
        assert!(output.contains("TransferSol"), "should have TransferSol instruction");
        assert!(output.contains("Assign"), "should have Assign instruction");
        assert!(output.contains("AdvanceNonceAccount"), "should have AdvanceNonceAccount instruction");
        assert!(output.contains("InitializeNonceAccount"), "should have InitializeNonceAccount instruction");
        // System program has nonce types
        let types_mod = extract_module(&output, "types");
        let has_nonce_type = types_mod.contains("Nonce");
        assert!(has_nonce_type, "types module should contain nonce-related types");
        // Bincode detection
        assert!(output.contains("repr (u32)"), "system program is bincode, unit enums should use repr(u32)");
    }
    ...
    fn test_pipeline_codama_token() {
        ...
        assert!(output.contains("InitializeMint"), "should have InitializeMint instruction");
        assert!(output.contains("Transfer"), "should have Transfer instruction");
        assert!(output.contains("Approve"), "should have Approve instruction");
        assert!(output.contains("MintTo"), "should have MintTo instruction");
        assert!(output.contains("Burn"), "should have Burn instruction");
        assert!(output.contains("CloseAccount"), "should have CloseAccount instruction");
        assert!(output.contains("FreezeAccount"), "should have FreezeAccount instruction");
        assert!(output.contains("TransferChecked"), "should have TransferChecked instruction");
        // Token has AuthorityType enum
        let types_mod = extract_module(&output, "types");
        assert!(types_mod.contains("AuthorityType"), "types should have AuthorityType enum");
    ...
    fn test_pipeline_codama_compute_budget() {
        ...
        assert!(output.contains("SetComputeUnitLimit"), "should have SetComputeUnitLimit instruction");
        assert!(output.contains("SetComputeUnitPrice"), "should have SetComputeUnitPrice instruction");
        assert!(output.contains("RequestHeapFrame"), "should have RequestHeapFrame instruction");
        // Minimal program: no custom types needed
        let ix_mod = extract_module(&output, "instruction");
        let disc_count = ix_mod.matches("impl anchor_lang :: Discriminator for").count();
        assert_eq!(disc_count, 5, "compute_budget should have 5 Discriminator impls, got {}", disc_count);
    }
    ...
    fn test_pipeline_codama_memo() {
        ...
        assert!(output.contains("AddMemo"), "should have AddMemo instruction");
        let ix_mod = extract_module(&output, "instruction");
        let disc_count = ix_mod.matches("impl anchor_lang :: Discriminator for").count();
        assert_eq!(disc_count, 1, "memo should have exactly 1 Discriminator impl, got {}", disc_count);
    }
    ...
    fn test_pipeline_anchor_stake() {
        ...
        assert!(output.contains("repr (u32)"), "anchor stake bincode enum should use repr(u32)");
    ...
    fn test_pipeline_anchor_marginfi() {
        ...
        assert!(!output.contains("repr (u32)"), "borsh IDL should not use repr(u32)");
        assert!(output.contains("repr (u8)"), "borsh IDL should use repr(u8)");
        // Should have WrappedI80F48 with From impls
        assert!(output.contains("WrappedI80F48"), "should have WrappedI80F48 type");
    ...
    fn test_pipeline_anchor_whirlpool() {
        ...
        assert!(output.contains("repr (C)"), "should have repr(C) for zero-copy types");
        assert!(output.contains("bytemuck"), "should have bytemuck derives for zero-copy types");

        // TickArray has [Tick; 88] which can't derive Default (>32)
        // Verify manual Default impl is generated
        assert!(output.contains("impl Default for TickArray"),
            "TickArray with [Tick;88] should get manual Default impl");

        // State module should have DISCRIMINATOR_LEN for account types
        assert!(output.contains("DISCRIMINATOR_LEN : usize = 8usize"),
    ...
    fn test_pipeline_shank_bubblegum() {
        ...
        assert!(output.contains("CreateTree"), "should have CreateTree instruction");
        assert!(output.contains("Delegate"), "should have Delegate instruction");
        assert!(output.contains("Compress"), "should have Compress instruction");
        // Key types
        let types_mod = extract_module(&output, "types");
        assert!(types_mod.contains("Creator"), "types should have Creator");
        assert!(types_mod.contains("MetadataArgs"), "types should have MetadataArgs");
        assert!(types_mod.contains("TokenStandard"), "types should have TokenStandard");
        assert!(types_mod.contains("LeafSchema"), "types should have LeafSchema");
        // Borsh encoding (Shank IDLs)
        assert!(!types_mod.contains("repr (u32)"), "Shank IDL should NOT have repr(u32)");
    }
    ...
    fn test_pipeline_shank_candy_guard() {
        ...
        // Key guard types
        let types_mod = extract_module(&output, "types");
        assert!(types_mod.contains("Allocation"), "types should have Allocation guard");
        assert!(types_mod.contains("AllowList"), "types should have AllowList guard");
        assert!(types_mod.contains("BotTax"), "types should have BotTax guard");
        assert!(types_mod.contains("EndDate"), "types should have EndDate guard");
    }
    ...
    fn test_pipeline_marginfi_state_module() {
        ...
        assert!(output.contains("pub struct Bank"), "state should have Bank struct");
        assert!(output.contains("pub struct MarginfiGroup"), "state should have MarginfiGroup struct");
        assert!(output.contains("pub struct MarginfiAccount"), "state should have MarginfiAccount struct");
    }
    ...
    fn test_pipeline_whirlpool_state_module() {
        ...
        assert!(output.contains("pub struct Whirlpool"), "state should have Whirlpool struct");
        assert!(output.contains("pub struct TickArray"), "state should have TickArray struct");
        assert!(output.contains("pub struct Position"), "state should have Position struct");
    }
    ...
    fn test_pipeline_all_types_generated() {
        ...
        for filename in &["anchor_marginfi.json", "anchor_whirlpool.json", "codama_stake.json"] {
            ...
            for ty in &idl.types {
                ...
                assert!(has_struct || has_enum || has_alias,
                    "Type {} from {} not found in generated output", ty.name, filename);
            }
        }
    }
    ...
    fn test_pipeline_program_id_generated() {
        for filename in &["anchor_marginfi.json", "codama_stake.json", "anchor_whirlpool.json"] {
            let output = run_full_pipeline(filename);
            assert!(output.contains("pub static ID"),
                "{} should generate program ID constant", filename);
            assert!(output.contains("Pubkey :: new_from_array"),
                "{} program ID should use new_from_array", filename);
        }
    }
    ...
    fn test_pipeline_marginfi_instruction_count() {
        ...
        let disc_count = output.matches("impl anchor_lang :: Discriminator for").count();
        assert_eq!(disc_count, 45,
            "should have 45 Discriminator impls (one per instruction), got {}", disc_count);
    }
    ...
    fn test_pipeline_whirlpool_zero_copy_count() {
        ...
        assert!(pod_count >= 5,
            "should have >= 5 Pod impls (for zero-copy types + state), got {}", pod_count);
    }
    ...
    fn test_pipeline_shank_bubblegum_state_module() {
        ...
        assert!(!idl.accounts.is_empty(),
            "bubblegum should have account entries, got {}", idl.accounts.len());
        ...
        for acc in &idl.accounts {
            if idl.types.iter().any(|t| t.name == acc.name) {
                assert!(state_mod.contains(&acc.name),
                    "state module should contain account type '{}', state module content: {}",
                    acc.name, &state_mod[..state_mod.len().min(500)]);
                // Verify it has DISCRIMINATOR (state module specific)
                assert!(state_mod.contains("DISCRIMINATOR"),
                    "state module accounts should have DISCRIMINATOR constant");
                found_any = true;
            }
        }
        assert!(found_any, "should have found at least one account in state module");
    }
    ...
    fn test_pipeline_shank_candy_guard_state_module() {
        let idl = parse_test_idl("shank_candy_guard.json");
        assert!(!idl.accounts.is_empty(),
            "candy_guard should have account entries, got {}", idl.accounts.len());
        ...
        for acc in &idl.accounts {
            if idl.types.iter().any(|t| t.name == acc.name) {
                assert!(state_mod.contains(&acc.name),
                    "state module should contain account type '{}' in state module", acc.name);
                found_any = true;
            }
        }
        assert!(found_any, "should have found at least one account in state module");
    ...
    fn test_idl_type_u256_maps_to_byte_array() {
        let output = codegen::idl_type_to_tokens(
            &anchor_lang_idl::types::IdlType::U256
        ).to_string();
    ...
    fn test_idl_type_i256_maps_to_byte_array() {
        let output = codegen::idl_type_to_tokens(
            &anchor_lang_idl::types::IdlType::I256
        ).to_string();
    ...
    fn test_pipeline_marginfi_defined_args() {
        ...
        let has_defined_arg = idl.instructions.iter().any(|ix| {
            ix.args.iter().any(|arg| matches!(&arg.ty,
                anchor_lang_idl::types::IdlType::Defined { .. }
            ))
        });
        assert!(has_defined_arg, "marginfi should have instructions with Defined type args");
        ...
        assert!(ix_mod.contains("BankConfigOpt"),
        ...
        assert!(ix_mod.contains("pub bank_config_opt : BankConfigOpt"),
    ...
    fn test_encoding_bincode_stake_unit_enums_are_repr_u32() {
        ...
        assert!(types_mod.contains("repr (u32)"),
            "bincode IDL unit enums should use repr(u32)");

        // Manual AnchorSerialize impl (not derived)
        assert!(types_mod.contains("impl AnchorSerialize for StakeAuthorize"),
            "bincode unit enum should have manual AnchorSerialize impl");
        assert!(types_mod.contains("impl AnchorDeserialize for StakeAuthorize"),
            "bincode unit enum should have manual AnchorDeserialize impl");
        assert!(types_mod.contains("to_le_bytes"),
            "bincode ser should write u32 LE");
        assert!(types_mod.contains("from_le_bytes"),
    ...
    fn test_encoding_bincode_anchor_stake_same_as_codama() {
        ...
        assert!(codama_types.contains("repr (u32)"), "codama stake should have repr(u32)");
        assert!(anchor_types.contains("repr (u32)"), "anchor stake should have repr(u32)");

        // Both should ALSO have repr(u8) — for data enums like StakeState
        // (data enums fall back to borsh-style even in bincode programs)
        assert!(codama_types.contains("repr (u8)"),
            "codama stake should have repr(u8) for data enums like StakeState");
        assert!(anchor_types.contains("repr (u8)"),
    ...
    fn test_encoding_borsh_marginfi_enums_are_repr_u8() {
        ...
        assert!(types_mod.contains("repr (u8)"),
        ...
        assert!(!types_mod.contains("impl AnchorSerialize for"),
    ...
    fn test_encoding_borsh_whirlpool_enums_are_repr_u8() {
        ...
        assert!(types_mod.contains("repr (u8)"), "whirlpool enums should use repr(u8)");
        assert!(!types_mod.contains("repr (u32)"), "whirlpool should NOT have repr(u32)");
    }
    ...
    fn test_encoding_bytemuck_on_repr_c_types() {
        ...
        assert!(types_mod.contains("repr (C)"), "whirlpool should have repr(C) types");
        assert!(types_mod.contains("bytemuck :: Pod"), "repr(C) types should get Pod");
        assert!(types_mod.contains("bytemuck :: Zeroable"), "repr(C) types should get Zeroable");
        ...
        assert!(marginfi_types.contains("repr (C)"),
            "marginfi has zero-copy account types → types module should have repr(C)");
        assert!(marginfi_types.contains("bytemuck :: Pod"),
    ...
    fn test_encoding_no_bytemuck_on_bincode_programs() {
        ...
        let codama_output = run_full_pipeline("codama_stake.json");
        let codama_types = extract_module(&codama_output, "types");
        assert!(!codama_types.contains("bytemuck"),
            "codama stake (bincode) should NOT have bytemuck");
        assert!(!codama_types.contains("repr (C)"),
            "codama stake should NOT have repr(C)");

        let anchor_output = run_full_pipeline("anchor_stake.json");
        let anchor_types = extract_module(&anchor_output, "types");
        assert!(!anchor_types.contains("bytemuck"),
    ...
    fn test_encoding_whirlpool_state_has_bytemuck_for_zero_copy_accounts() {
        ...
        assert!(state_mod.contains("bytemuck :: Pod"),
            "whirlpool state module should have bytemuck Pod for zero-copy accounts");
        assert!(state_mod.contains("repr (C)"),
    ...
    fn test_encoding_discriminator_lengths_match_encoding() {
        ...
        for ix in &stake.instructions {
            assert_eq!(ix.discriminator.len(), 4,
                "bincode instruction '{}' should have 4-byte disc", ix.name);
        }
        ...
        for ix in &marginfi.instructions {
            assert_eq!(ix.discriminator.len(), 8,
                "borsh instruction '{}' should have 8-byte disc", ix.name);
        }
        ...
        assert_eq!(disc_count, 18,
            "stake should have 18 DISCRIMINATOR consts, got {}", disc_count);

        let marginfi_output = run_full_pipeline("anchor_marginfi.json");
        let marginfi_ix = extract_module(&marginfi_output, "instruction");
        let disc_count = marginfi_ix.matches("DISCRIMINATOR : & 'static [u8]").count();
        assert_eq!(disc_count, 45,
            "marginfi should have 45 DISCRIMINATOR consts, got {}", disc_count);
    }
    ...
    fn test_encoding_bincode_data_enum_falls_back_to_borsh() {
        ...
        assert!(types_mod.contains("pub enum StakeState"),
        ...
        assert!(types_mod.contains("repr (u32)"), "should have repr(u32) for unit enums");
        assert!(types_mod.contains("repr (u8)"), "should have repr(u8) for data enums");
    }
    ...
    fn test_encoding_shank_idls_use_borsh() {
        ...
        for filename in &["shank_bubblegum.json", "shank_candy_guard.json"] {
            ...
            let use_bincode = idl.instructions.first()
                .map(|ix| ix.discriminator.len() == 4)
                .unwrap_or(false);
            assert!(!use_bincode, "{} should be detected as borsh, not bincode", filename);

            let output = run_full_pipeline(filename);
            let types_mod = extract_module(&output, "types");
            assert!(!types_mod.contains("repr (u32)"),
                "{} should NOT have repr(u32) in types (borsh program)", filename);
        }
    }
    ...
    fn assert_valid_rust_syntax(filename: &str) {
        ...
        syn::parse2::<syn::File>(tokens).unwrap_or_else(|e| {
            panic!("Generated code for {} is not valid Rust syntax: {}", filename, e);
        });
    }

    #[test]
    fn test_syntax_valid_codama_stake() { assert_valid_rust_syntax("codama_stake.json"); }
    #[test]
    fn test_syntax_valid_codama_system() { assert_valid_rust_syntax("codama_system.json"); }
    #[test]
    fn test_syntax_valid_codama_token() { assert_valid_rust_syntax("codama_token.json"); }
    #[test]
    fn test_syntax_valid_codama_compute_budget() { assert_valid_rust_syntax("codama_compute_budget.json"); }
    #[test]
    fn test_syntax_valid_codama_memo() { assert_valid_rust_syntax("codama_memo.json"); }
    #[test]
    fn test_syntax_valid_anchor_stake() { assert_valid_rust_syntax("anchor_stake.json"); }
    #[test]
    fn test_syntax_valid_anchor_marginfi() { assert_valid_rust_syntax("anchor_marginfi.json"); }
    #[test]
    fn test_syntax_valid_anchor_whirlpool() { assert_valid_rust_syntax("anchor_whirlpool.json"); }
    #[test]
    fn test_syntax_valid_shank_bubblegum() { assert_valid_rust_syntax("shank_bubblegum.json"); }
    #[test]
    fn test_syntax_valid_shank_candy_guard() { assert_valid_rust_syntax("shank_candy_guard.json"); }
    ...
    fn test_instruction_data_method_contains_discriminator_prepend() {
        ...
        let data_impl_count = stake_ix.matches("impl anchor_lang :: InstructionData for").count();
        assert_eq!(data_impl_count, 18,
            "stake should have 18 InstructionData impls, got {}", data_impl_count);

        // Borsh (8-byte disc)
        let marginfi_output = run_full_pipeline("anchor_marginfi.json");
        let marginfi_ix = extract_module(&marginfi_output, "instruction");
        let data_impl_count = marginfi_ix.matches("impl anchor_lang :: InstructionData for").count();
        assert_eq!(data_impl_count, 45,
            "marginfi should have 45 InstructionData impls, got {}", data_impl_count);
    }
    ...
    fn test_instruction_discriminator_byte_values_correct() {
        ...
        for byte in &ix0.discriminator {
            ...
            assert!(ix_mod.contains(&byte_str),
                "instruction '{}' discriminator should contain byte {}, ix_mod snippet: {}",
                ix0.name, byte_str, &ix_mod[..ix_mod.len().min(1000)]);
        }
        ...
        idl.instructions.iter()
            .find(|ix| ix.name == "lending_account_deposit")
            .expect("should have lending_account_deposit");
        assert!(ix_mod.contains("171u8"), "deposit disc should contain 171u8");
        assert!(ix_mod.contains("140u8"), "deposit disc should contain 140u8");
    }
    ...
    fn test_instruction_args_field_types_match_idl() {
        ...
        assert_eq!(deposit.args[0].name, "amount");
        assert_eq!(deposit.args[1].name, "deposit_up_to_limit");
        assert!(ix_mod.contains("pub amount : u64"),
            "deposit should have amount: u64 field");
        assert!(ix_mod.contains("pub deposit_up_to_limit : Option < bool >"),
