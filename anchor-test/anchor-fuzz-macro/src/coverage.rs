...
pub fn coverage_state_code() -> proc_macro2::TokenStream {
    quote! {
        ...
        pub struct CoverageState {
            ...
            pub branch_outcomes: anchor_test_context::FastHashMap<u64, anchor_test_context::FastHashMap<(usize, bool), u64>>,
            // PC hit tracking for source-level LCOV (only when --coverage enabled)
            pub pc_hits: anchor_test_context::FastHashMap<u64, anchor_test_context::FastHashMap<usize, u64>>,
    ...
    quote! {
        ...
        impl FuzzCallback {
            ...
            fn flush_bitmap_updates_large(bitmap: *mut u8, updates: &[(usize, u8)], bitmap_size: usize) {
                let mut merged: anchor_test_context::FastHashMap<usize, u8> =
                    anchor_test_context::FastHashMap::default();
