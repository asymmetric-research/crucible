use {
    ...
    solana_program_runtime::{
        loaded_programs::{LoadProgramMetrics, ProgramCacheEntry, ProgramCacheForTxBatch, ProgramCache},
...
impl AccountsDb {
    ...
    pub(crate) fn delete_accounts(
        &mut self,
        pubkeys: Vec<Pubkey>,
    ) -> Result<(), LiteSVMError> {
        //might be sufficient...¯\_(ツ)_/¯ 

        //remove pubkeys
        pubkeys.iter().for_each(|x| {self.inner.remove(x);});

        //reset cache
        self.programs_cache = ProgramCacheForTxBatch::new(
            self.programs_cache.slot(),
            self.programs_cache.environments.clone(),
            self.programs_cache.upcoming_environments.clone(),
            self.programs_cache.latest_root_epoch,
        );
