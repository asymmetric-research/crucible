...
impl LiteSVM {
    ...
    pub fn get_all_account_keys(&self) -> Vec<Pubkey> {
        // This accesses the internal accounts map
        self.accounts.inner.keys().cloned().collect()
    }
    ...
    pub fn delete_accounts(&mut self, pubkeys: Vec<Pubkey>) -> Result<(), LiteSVMError> {
        self.accounts.delete_accounts(pubkeys)
    }
        ...
        match maybe_program_indices {
            Ok(program_indices) => {
                ...
                let mut tx_result = {
                    ...
                    //Ok if the transaction succeeds else returns TransactionError
                    ...
                    if let Some(trace_collector) = trace_collector {
                        //might be worth it to pass result here as well
                        trace_collector
                            .borrow_mut()
                            .trace(tx.message(), invoke_context.get_traces());
                    }
    ...
    pub fn send_transaction(&mut self, tx: impl Into<VersionedTransaction>) -> TransactionResult {
        ...
        let ExecutionResult {
            ...
            self.execute_transaction_no_verify(
                vtx,
                log_collector.clone(),
    ...
    pub fn simulate_transaction(
        ...
        let ExecutionResult {
            ...
            self.execute_transaction_readonly(
                tx.into(),
                log_collector.clone(),
                self.trace_collector.clone(),
            )
        } else {
            self.execute_transaction_no_verify_readonly(
                tx.into(),
                log_collector.clone(),
                self.trace_collector.clone(),
            )
