//! CLI integration tests for anchor fuzz commands
//!
//! Tests cover:
//! - `anchor fuzz init` - harness creation
//! - `anchor fuzz run` - fuzzing execution modes
//! - `anchor fuzz list` - test discovery
//! - `anchor fuzz show` - crash inspection
//! - `anchor fuzz cmin` - corpus minimization
...
/// Get the project root (where the cli/ directory is)
...
/// Run anchor fuzz command in a directory
fn run_anchor_fuzz_in(dir: &Path, args: &[&str]) -> std::process::Output {
    let anchor_bin = project_root().join("target/debug/anchor");

    let mut cmd = Command::new(&anchor_bin);
    cmd.current_dir(dir)
        .arg("fuzz")
        .args(args)
        .stdout(Stdio::piped())
    ...
    cmd.output().expect("Failed to execute anchor fuzz command")
}

/// Check that anchor CLI is built
fn ensure_cli_built() {
    let anchor_bin = project_root().join("target/debug/anchor");
    if !anchor_bin.exists() {
        panic!(
            "Anchor CLI not found at {}. Build it first with: cargo build -p anchor-cli",
            anchor_bin.display()
        );
    }
}
...
fn test_init_creates_workspace() {
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["init", "my_program"]);
...
fn test_init_idempotent() {
    ...
    let output1 = run_anchor_fuzz_in(temp.path(), &["init", "my_program"]);
    assert!(output1.status.success(), "first init should succeed");

    // Second init should fail (directory already exists)
    let output2 = run_anchor_fuzz_in(temp.path(), &["init", "my_program"]);
...
fn test_run_dry_run() {
    ...
    let marginfi_path = project_root().join("fuzz/examples/marginfi-v2-fuzz");
    if !marginfi_path.exists() {
        eprintln!("Skipping test_run_dry_run: marginfi example not found");
        return;
    }

    let output = run_anchor_fuzz_in(&marginfi_path, &["run", "marginfi-v2-fuzz", "invariant_test", "--dry-run"]);
...
fn test_run_nonexistent_harness() {
    ...
    let temp = TempDir::new().unwrap();
    let output = run_anchor_fuzz_in(temp.path(), &["run", "nonexistent", "test", "--dry-run"]);
...
"#,
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["run", "test_prog", "test_feature", "--timeout", "30", "--dry-run"]);
...
fn test_list_no_fuzz_dir() {
    ...
    let temp = TempDir::new().unwrap();
    let output = run_anchor_fuzz_in(temp.path(), &["list"]);
...
"#,
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["list", "my_prog"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_list_nonexistent_program() {
    ...
    let temp = TempDir::new().unwrap();
    let output = run_anchor_fuzz_in(temp.path(), &["list", "nonexistent"]);
...
"#,
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["show", "my_prog"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
"#,
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["show", "my_prog", "crash_xyz"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
"#,
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["show", "my_prog", "nonexistent_crash"]);
...
"#,
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["show", "my_prog", "crash_replay", "--replay"]);
...
fn test_init_special_characters_in_name() {
    ...
    let output = run_anchor_fuzz_in(temp.path(), &["init", "my_program_v2"]);
...
fn ensure_test_program_built() -> bool {
    ...
    if !binary.exists() {
        eprintln!(
            ...
            Build it first:\n\
              cd anchor-test/test-program && cargo build-sbf\n\
              cd anchor-test/test-program/fuzz/test-program-fuzz && cargo build --release --features invariant_test\n\
            ========================================================\n"
        );
        return false;
    }
