//! Common test utilities for anchor-fuzz CLI tests

use std::path::{Path, PathBuf};
use std::process::{Command, Output};
use tempfile::TempDir;

/// Get the path to the anchor CLI binary
pub fn anchor_binary() -> PathBuf {
    // The anchor CLI is in the parent workspace
    let workspace_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap()
        .to_path_buf();
    workspace_root.join("target/debug/anchor")
}

/// Run anchor fuzz command with given arguments
pub fn run_anchor_fuzz(args: &[&str], cwd: Option<&Path>) -> Output {
    let mut cmd = Command::new(anchor_binary());
    cmd.arg("fuzz");
    cmd.args(args);
...
/// Run anchor fuzz command and return stdout as string
pub fn run_anchor_fuzz_stdout(args: &[&str], cwd: Option<&Path>) -> String {
    let output = run_anchor_fuzz(args, cwd);
    String::from_utf8_lossy(&output.stdout).to_string()
}

/// Run anchor fuzz command and return stderr as string
pub fn run_anchor_fuzz_stderr(args: &[&str], cwd: Option<&Path>) -> String {
    let output = run_anchor_fuzz(args, cwd);
