...
use syn::{
    parse_macro_input, ItemImpl, ImplItem, ImplItemFn, FnArg, PatType,
    ItemFn, parse::Parse, parse::ParseStream, Token, Ident, Type, Path,
};
...
pub fn invariant_test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let fixture_type = &args.setup_path.segments.first()
