...
LiteSVM 0.15 uses modular Solana/Agave crates whose compatible dependency graph spans more than
one major version. Use the dependencies generated by `crucible init`; do not force every
`solana-*` crate to one version merely to make the numbers match.

Use the constant exported by the program-ID module when that is what the selected crate version
provides:
...
Sysvar account data is wincode-encoded in the Solana 4 runtime. Update Clock and other sysvars
through `ctx.set_sysvar(...)` so LiteSVM's account data and sysvar cache stay coherent. This does
not change native instruction data that is defined as bincode by the target program.

`ctx.add_program(...)` creates an immutable upgradeable-loader Program/ProgramData pair. A test
requiring a non-`None` upgrade authority must clone or explicitly seed matching loader accounts.
See the [LiteSVM 0.15.2 migration guide](litesvm-0.15-migration.md) for the complete compatibility
notes.
